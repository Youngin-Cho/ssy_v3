    ...
    with torch.no_grad():
        ...
        for path in val_paths:
            test_env = SteelStockYard(val_dir + path, look_ahead=look_ahead,
                                      ...
                                      algorithm="RL", record_events=False, device=device)

            state, mask, crane_id = test_env.reset()
            done = False

            while not done:
                action, _ = agent.actor.act(state, mask, crane_id, greedy=True)
...
if __name__ == "__main__":
    ...
    agent = Agent(env.meta_data, env.state_size, env.num_nodes, embed_dim, num_heads,
                  num_HGT_layers, num_actor_layers, num_critic_layers, lr, lr_decay, lr_step, gamma,
                  buffer_size, batch_size, min_buffer_size, tau,
                  alpha_init, target_entropy_ratio, use_gnn=use_gnn, device=device)
    if cfg.vessl == 0:
    ...
    if cfg.load_model:
        ...
        agent.critic1_target.load_state_dict(checkpoint['critic1_state_dict'])
        agent.critic2_target.load_state_dict(checkpoint['critic2_state_dict'])
        agent.actor_optimizer.load_state_dict(checkpoint['actor_optimizer_state_dict'])
        agent.critic_optimizer.load_state_dict(checkpoint['critic_optimizer_state_dict'])
        agent.log_alpha = checkpoint['log_alpha'].to(device).requires_grad_()
    else:
        start_episode = 1
    ...
    for e in range(start_episode, n_episode + 1):
        if cfg.vessl == 1:
            vessl.log(payload={"Train/learnig_rate": agent.actor_scheduler.get_last_lr()[0]}, step=e)
        elif cfg.vessl == 0:
            writer.add_scalar("Training/Learning Rate", agent.actor_scheduler.get_last_lr()[0], e)
        ...
        while not done:
            action, _ = agent.get_action(state, mask, crane_id)
            next_state, reward, done, next_mask, next_crane_id = env.step(action)

            r_epi += reward
            agent.put_data((state, action, reward, next_state, mask, next_mask, done), None)
        ...
        with open(log_dir + "train_log.csv", 'a') as f:
            f.write('%d, %1.4f, %1.4f, %f\n' % (e, r_epi, avg_loss, agent.actor_scheduler.get_last_lr()[0]))
