...
def evaluate(val_dir):
    ...
    with torch.no_grad():
        ...
        for path in val_paths:
            ...
            while not done:
                action, _ = agent.network.act(state, mask, crane_id, agent.num_quantiles, epsilon=0.0)
...
if __name__ == "__main__":
    ...
    agent = Agent(env.meta_data, env.state_size, env.num_nodes, embed_dim, num_heads,
                  num_HGT_layers, num_q_layers, n_cos, num_quantiles, kappa, lr, lr_decay, lr_step, gamma,
                  buffer_size, batch_size, min_buffer_size, target_update,
                  epsilon_start, epsilon_end, epsilon_decay, use_gnn=use_gnn, device=device)
    if cfg.vessl == 0:
